...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!accessKey || accessKey === "YOUR_ACCESS_KEY_HERE" || accessKey.trim() === "") {
      // Simulate form submission to allow testing the flow in local development
      setTimeout(() => {
        setSubmitted(true);
        setFormValues({ name: '', email: '', message: '' });
        setIsSubmitting(false);
        console.info(
          "Form submission simulated successfully! To receive actual emails, get a free key from https://web3forms.com and configure VITE_WEB3FORMS_ACCESS_KEY in your .env file."
        );
      }, 1200);
      return;
    }
          ...
          {submitted ? (
            ...
            >
              ...
              <div className="relative group flex flex-col">
                ...
                <label
                  htmlFor="name"
                  className={`absolute left-0 transition-all duration-300 pointer-events-none text-[13px] md:text-sm font-light ${focusedField === 'name' || formValues.name.trim() !== ''
              ...
              <div className="relative group flex flex-col">
                ...
                <label
                  htmlFor="email"
                  className={`absolute left-0 transition-all duration-300 pointer-events-none text-[13px] md:text-sm font-light ${focusedField === 'email' || formValues.email.trim() !== ''
                      ? '-top-2 text-[9px] text-[var(--color-primary-accent)] font-bold tracking-wider uppercase'
                      : 'top-4 text-[var(--color-secondary-text)]'
                    }`}
                >
              ...
              <div className="relative group flex flex-col">
                ...
                <label
                  htmlFor="message"
                  className={`absolute left-0 transition-all duration-300 pointer-events-none text-[13px] md:text-sm font-light ${focusedField === 'message' || formValues.message.trim() !== ''
                      ? '-top-2 text-[9px] text-[var(--color-primary-accent)] font-bold tracking-wider uppercase'
                      : 'top-4 text-[var(--color-secondary-text)]'
                    }`}
                >
